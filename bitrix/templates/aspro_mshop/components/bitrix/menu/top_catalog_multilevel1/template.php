				...
				<li class="menu_item_l1 <?=(!$key ? ' first' : '')?><?=($arItem["SELECTED"] ? ' current' : '')?><?=($arItem["PARAMS"]["ACTIVE"]=="Y" ? ' active' : '')?>">
					...
					<?if($arItem["IS_PARENT"]):?>
						<div class="child submenu">
                            <? 
                                $img = CFile::ResizeImageGet( $arItem['PARAMS']['PICTURE'], array( "width" => 560, "height" => 400 ), BX_RESIZE_IMAGE_EXACT, true, array() ); 
                            ?>
							...
							<div class="child_wrapp">
								...
								<?foreach($arItem["CHILD"] as $i => $arSubItem):?>
									<?if(count($arSubItem["CHILD"])):?>
										<div class="depth3<?=($i > 8 ? ' d' : '')?>"<?=($i > 8 ? ' style="display:none;"' : '')?>>
                                            <? $img = CFile::ResizeImageGet( $arSubItem['PARAMS']['PICTURE'], array( "width" => 255, "height" => 80 ), BX_RESIZE_IMAGE_EXACT, true, array() ); 
                                            ?>
											<a class="title<? if($img['src']){ echo ' depth3_title_image'; }?><?=($arSubItem["SELECTED"] ? ' current' : '')?>" href="<?=$arSubItem["LINK"]?>">
                                            <? if($img['src']){?>
                                                <span class="img" style="background-image: url(<? echo $img['src'];?>);"></span>
                                            <? }?>
											...
											<?if($arSubItem["CHILD"] && is_array($arSubItem["CHILD"])):?>
												<?foreach($arSubItem["CHILD"] as $ii => $arSubItem3):?>
													<a class="<?=($arSubItem3["SELECTED"] ? ' current' : '')?><?=($ii > 8 ? ' d' : '')?>" href="<?=$arSubItem3["LINK"]?>"<?=($ii > 8 ? ' style="display:none;"' : '')?>><?=$arSubItem3["TEXT"]?></a>
												<?endforeach;?>
												<?if(count($arSubItem["CHILD"]) > 9):?>
													<!--noindex-->
									...
									<?else:?>
										<a class="<?=($arSubItem["SELECTED"] ? ' current' : '')?><?=($i > 8 ? ' d' : '')?>" href="<?=$arSubItem["LINK"]?>"<?=($i > 8 ? ' style="display:none;"' : '')?>><?=$arSubItem["TEXT"]?></a>
									<?endif;?>
								<?endforeach;?>
                                
								<?if(count($arItem["CHILD"]) > 9):?>
									<!--noindex-->
						...
						<?foreach($arResult as $key => $arItem):?>
							<?if($key >= $maxRootItems):?>
								<ul<?=(($key - $maxRootItems) % 3 ? '' : ' class="last"')?>>
                                    <? $img = CFile::ResizeImageGet( $arItem['PARAMS']['PICTURE'], array( "width" => 255, "height" => 80 ), BX_RESIZE_IMAGE_EXACT, true, array() ); 
                                    ?>
									<li class="menu_title<?=($arItem["SELECTED"] ? ' current' : '')?><?=($arItem["PARAMS"]["ACTIVE"] == "Y" ? ' active' : '')?>">
                                       
										<a<? if($img['src']){ echo ' class="depth3_title_image"'; }?> href="<?=$arItem["LINK"]?>">
                                            <? if($img['src']){?>
                                                <span class="img" style="background-image: url(<? echo $img['src'];?>);"></span>
                                            <? }?>
