...
if($params['PS_IS_TEST']=='Y'){
    $params['URL']='https://test.paysec.by/pay/order.cfm';
}else{
    $params['URL']='https://pay149.paysec.by/pay/order.cfm';
}
if(!empty($params['BUYER_PERSON_NAME_FIRST'])){
    $fio=explode(' ', $params['BUYER_PERSON_NAME_FIRST']);
}
//pr($params);?>
...
<input type="hidden" name="Firstname" value="<?=$fio['0']?>">
<input type="hidden" name="Middlename" value="<?=$fio['3'];?>">
...
<input TYPE="SUBMIT" name="Submit" class='red-button btn' value="<?=Loc::getMessage("SALE_HANDLERS_PAY_SYSTEM_ASSIST_ACTION");?>"><br><br>
</form>
